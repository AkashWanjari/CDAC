...
//	
	...
	@Query(value="select max(bid_price) as bid_price,buyer_id from Biddings  where pid=?1",nativeQuery = true)
	public List<Object> getHighestBid(Biddings pid);
...
//	public List<Biddings> getProductBids(long prdctid);
	...
	@Query(value="select max(bid_price) as bid_price from Biddings  where pid=?1",nativeQuery = true)
	public List<Object> getBiddshighest(String byrId);
